...
private let assertionMessageMainThread = "Expected to only be called on the main thread"

public class PBBannerView: UIView,
    ...
    DisplayViewInteractionDelegate
{
    ...
    @objc public var bannerParameters: BannerParameters { adUnitConfig.adConfiguration.bannerParameters }

    @objc public var videoParameters: VideoParameters { adUnitConfig.adConfiguration.videoParameters }
    ...
    {
        self.adUnitConfig = AdUnitConfig(configId: configID, size: adSize)
        adUnitConfig.adConfiguration.bannerParameters.api = PrebidConstants.supportedRenderingBannerAPISignals

        self.eventHandler = eventHandler
        super.init(frame: frame)
        accessibilityLabel = PBMAccesibility.pbBannerView

        let bannerAdLoader = PBMBannerAdLoader(delegate: self)

        self.adLoadFlowController = PBMAdLoadFlowController(
            bidRequesterFactory: { [adUnitConfig] _ in
            ...
            delegate: self,
            configValidationBlock: { _, _ in
                true
            }
        )

        self.autoRefreshManager = PBMAutoRefreshManager(
    ...
    public func trackImpression(forDisplayView: PBMDisplayView) {
        guard let eventHandler = eventHandler,
              eventHandler.responds(to: #selector(BannerEventHandler.trackImpression))
        else {
            return
        }
    ...
    public var viewControllerForPresentingModal: UIViewController? {
        guard let delegate = delegate,
              delegate.responds(to: #selector(BannerViewDelegate.bannerViewPresentationController))
        else {
            return nil
        }
    ...
    private func reportLoadingSuccess(with size: CGSize) {
        DispatchQueue.main.async { [weak self] in
            guard let self = self,
                  ...
                  delegate.responds(to: #selector(BannerViewDelegate.bannerView(_:didReceiveAdWithAdSize:)))
            else {
                return
            }
